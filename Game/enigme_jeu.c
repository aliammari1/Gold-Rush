...
//pour compiler 
//// gcc main.c -o pr0 -lSDL -lSDL_image -lSDL_ttf -lSDL_mixer 
/**  
* @file main.c  
...
while(cont==1){//ki tenzel echap yokhrej w cont=0 
	if(score<=0)
	cont=0;////
	//affichage
	...
	afficherenigme(e,screen);
		...
		/////animation
		for(int i=0;i<2;i++){//nbr de ligne fl matrice(matrcie)
			for(int j=0;j<9;j++){
				SDL_Flip(screen);
		...
		case SDLK_c:
            {	
		if(detect_reponse==0){
			if(e.reponse!=3){////////////////lfar9 bin les case houni
				score-=20;	
				detect_reponse=0;			
				init_enigmes(&e);
				sprintf(scorechar,"%d",score);
