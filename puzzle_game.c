...
void enigme_jeu()
{
	...
	initPuzzle(&puzzle); /// bhs taateha enigme e w traajaa l enigme meebi
	...
	{ // ki tenzel echap yokhrej w cont=0
		...
		case SDL_KEYDOWN:
			...
			case SDLK_a:
			{
				if (detect_reponse == 0)
				{
					if (puzzle.correctAnswer != 1)
					{
						score -= 20;
						initPuzzle(&puzzle);				 // bsh ychargilk les questions taa sou1el li baad
						...
						{ // nbr de ligne fl matrice(matrcie)
							...
							{
								SDL_Flip(screen);
								animatePuzzle(&puzzle,  &pos_sprite, j); /// fct animer
					...
					else if (puzzle.correctAnswer == 1)
					{
						score += 20;
						initPuzzle(&puzzle);
				...
				if (detect_reponse == 0)
				{
					if (puzzle.correctAnswer != 2)
					{
						...
						for (int i = 0; i < 2; i++)
						{
							...
							{
								SDL_Flip(screen);
								animatePuzzle(&puzzle,  &pos_sprite, j);
								SDL_BlitSurface(skull, &pos_sprite, screen, &pos_screen);
								SDL_BlitSurface(surface_message, NULL, screen, &position_message);
								SDL_Flip(screen);
								SDL_Delay(100);
					...
					else if (puzzle.correctAnswer == 2)
					{
						score += 20;
						sprintf(scorechar, "%d", score);
						surface_total_score = TTF_RenderText_Blended(police, scorechar, blue);
			...
			case SDLK_c:
			{
				if (detect_reponse == 0)
				{
					...
					else if (puzzle.correctAnswer == 3)
					{
						score += 20;
						sprintf(scorechar, "%d", score);
						surface_total_score = TTF_RenderText_Blended(police, scorechar, blue);
		...
		case SDL_MOUSEMOTION:
		{
			if (event.motion.x > 500 && event.motion.x < 550 && event.motion.y > 150 && event.motion.y < 170) // reponse 1
			{
				if (detect_sound == 0)
				{
					...
					detect_sound = 1;
					puzzle.answer1Surface = TTF_RenderText_Blended(police, puzzle.answer1, yellow);
					puzzle.answer2Surface = TTF_RenderText_Blended(police, puzzle.answer2, white);
					puzzle.answer3Surface = TTF_RenderText_Blended(police, puzzle.answer3, white);
				}
			}
			...
			else if (event.motion.x > 500 && event.motion.x < 550 && event.motion.y > 250 && event.motion.y < 270) // reponse2
			{
				...
				puzzle.answer1Surface = TTF_RenderText_Blended(police, puzzle.answer1, white);
				puzzle.answer2Surface = TTF_RenderText_Blended(police, puzzle.answer2, yellow);
				puzzle.answer3Surface = TTF_RenderText_Blended(police, puzzle.answer3, white);
			}
			...
			else if (event.motion.x > 500 && event.motion.x < 550 && event.motion.y > 350 && event.motion.y < 370)
			{
				...
				puzzle.answer3Surface = TTF_RenderText_Blended(police, puzzle.answer3, yellow);
				puzzle.answer1Surface = TTF_RenderText_Blended(police, puzzle.answer1, white);
				puzzle.answer2Surface = TTF_RenderText_Blended(police, puzzle.answer2, white);
			}
			else
